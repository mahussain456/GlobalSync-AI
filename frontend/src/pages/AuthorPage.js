  ...
  return (
    <div className="min-h-screen bg-paper text-ink relative">
      ...
      <article className="max-w-4xl mx-auto px-6 pt-16 pb-12">
        <div className="bg-surface  rounded-xl border border-line p-8 flex flex-col md:flex-row items-center md:items-start gap-8 mb-12">
          <div className="w-32 h-32 bg-gem-gold/20 text-pine rounded-full flex items-center justify-center font-bold text-4xl shrink-0 border border-line">
            {author.avatar}
          </div>
          <div className="text-center md:text-left">
            ...
            <div className="text-pine font-medium mb-4">{author.role}</div>
            <p className="text-quiet leading-relaxed max-w-2xl">
              {author.bio}
            </p>
          </div>
        </div>
        ...
        <div className="bg-surface  rounded-xl border border-line p-8 md:p-10 text-quiet text-sm leading-relaxed mb-12 space-y-6">
          <h2 className="font-heading text-2xl font-bold text-pine">Why I Built GlobalSync AI</h2>
          <p>
          ...
          <h2 className="font-heading text-2xl font-bold text-pine pt-4">My Background & Expertise</h2>
          <p>
          ...
          <p>
            We maintain strict quality, research, and review standards across all our published guides. You can learn more about our commitment to editorial independence on our <Link to="/editorial-policy" className="text-pine hover:underline">Editorial Policy</Link> page, discover our brand background on the <Link to="/about" className="text-pine hover:underline">About Us</Link> page, or submit feedback and bug reports directly via our <Link to="/contact" className="text-pine hover:underline">Contact Page</Link>.
          </p>
        </div>

        <h2 className="font-heading text-2xl font-bold text-ink mb-6 flex items-center gap-2">
          <BookOpen className="w-5 h-5 text-pine" /> Articles by {author.name}
        </h2>
          ...
          {authorPosts.map(post => {
            ...
            return (
              <Link
                key={post.slug}
                to={`/blog/${post.slug}`}
                className="bg-surface  rounded-xl border border-line p-6 hover:border-line transition-all group flex flex-col h-full"
              >
                <div className="mb-4">
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full border ${style.badge} mb-3 inline-block`}>{post.category}</span>
                  <h3 className="font-bold text-lg text-ink leading-snug group-hover:text-quiet transition-colors mb-2">{post.title}</h3>
                  <p className="text-sm text-quiet line-clamp-2 leading-relaxed">{post.excerpt}</p>
                </div>
                <div className="mt-auto pt-4 border-t border-line flex items-center justify-between text-xs text-quiet">
