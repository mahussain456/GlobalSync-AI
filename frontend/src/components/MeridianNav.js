...
export default function MeridianNav(){
 ...
 useEffect(()=>{const skip=header.current.querySelector('.brand-skip');const jump=(event)=>{event.preventDefault();const main=document.querySelector('main');if(main){main.id='main-content';main.tabIndex=-1;main.focus()}};skip.addEventListener('click',jump);return()=>skip.removeEventListener('click',jump)},[]);
 ...
 return <header ref={header} className={`meridian-site-nav ${scrolled?'is-scrolled':''}`}><a className="brand-skip" href="#main-content">Skip to content</a><nav aria-label="Main navigation" className="brand-nav-inner"><Link to="/" className="brand-wordmark" aria-label="GlobalSync AI home"><img src="/meridian/logo-finished.png" alt="globalsync-ai" width="2172" height="724"/></Link><div className="brand-desktop-links">{links.map(([label,to])=><Link key={to} to={to} aria-current={pathname===to?'page':undefined}>{label}</Link>)}</div><Link className="brand-workspace" to="/dashboard" rel="nofollow">Open workspace <ArrowRight size={16}/></Link><button ref={button} className="brand-menu" aria-label="Toggle menu" aria-expanded={open} aria-controls="brand-mobile-nav" onClick={()=>setOpen(v=>!v)}>{open?<X/>:<Menu/>}</button></nav>{open&&<nav id="brand-mobile-nav" aria-label="Mobile navigation" className="brand-mobile-links">{links.map(([label,to])=><Link key={to} to={to} aria-current={pathname===to?'page':undefined} onClick={()=>setOpen(false)}>{label}</Link>)}<Link to="/dashboard" onClick={()=>setOpen(false)} rel="nofollow">Open workspace</Link><div className="brand-saved-teams"><SavedTeamsPanel/></div></nav>}</header>;
}
