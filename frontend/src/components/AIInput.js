import { localParseQuery } from "@/lib/parseQuery";
...
const INTENT_LABELS = {
  time_conversion: { label: "Time Zone", color: "bg-paper text-ink border-line " },
  meeting_overlap: { label: "Meeting Overlap", color: "bg-paper text-ink border-line " },
  currency_conversion: { label: "Currency", color: "bg-paper text-ink border-line " },
};
...
export default function AIInput({ onResult, initialQuery = "", autoSubmit = false, onAutoSubmitDone }) {
  ...
  const handleSubmit = async (q = query) => {
    ...
    try {
      ...
      onResult?.({ ...result, originalQuery: text });
      onAutoSubmitDone?.();
  ...
  return (
    <div data-testid="ai-input-section" className={`ai-glow-wrap${isFocused ? " focused" : ""}`}>
      <div className="bg-surface  rounded-xl p-3  border border-line transition-all">
        <div className="flex items-center gap-3">
          <div className="flex-1 relative">
            <div className="absolute left-3.5 top-1/2 -translate-y-1/2 w-7 h-7 rounded-lg bg-pine text-paper flex items-center justify-center">
            ...
            <input
              ...
              className="w-full pl-14 pr-10 py-3.5 bg-transparent text-ink placeholder-gem-mist/50 outline-none text-sm sm:text-base font-medium"
            ...
            {query && (
              <button
                ...
                className="absolute right-3 top-1/2 -translate-y-1/2 text-quiet hover:text-ink"
                aria-label="Clear query" data-testid="ai-input-clear"
              >
                <X className="w-4 h-4" />
              </button>
            )}
          ...
          <Button
            ...
            className="rounded-xl bg-pine text-paper px-5 py-3 font-semibold flex items-center gap-2 shrink-0 border-0  hover:opacity-90 transition-all"
      ...
      {intentMeta && (
        <div className="mt-2.5 flex items-center gap-2" data-testid="ai-intent-badge">
          ...
          <span className="text-xs text-quiet">Routing to the correct tool...</span>
        </div>
      )}
    </div>
  );
