...
function PlannerForm({ search }) {
  ...
  return <section aria-label="Interactive meeting planner" data-testid="meeting-planner" className="relative rounded-2xl border border-line bg-wash p-4 sm:p-7">
    <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 mb-7">
      <label className="space-y-2 text-sm text-quiet">Meeting date in {reference.name}<input aria-label="Meeting date" type="date" value={plan.date} onChange={e => update({ date: e.target.value })} className={inputClass} /></label>
      <label className="space-y-2 text-sm text-quiet">Meeting duration<select aria-label="Meeting duration" value={plan.duration} onChange={e => update({ duration: Number(e.target.value) })} className={inputClass}>{DURATIONS.map(minutes => <option key={minutes} value={minutes}>{minutes} minutes</option>)}</select></label>
      <label className="space-y-2 text-sm text-quiet">Time format<select aria-label="Time format" value={hour12 ? '12' : '24'} onChange={e => setHour12(e.target.value === '12')} className={inputClass}><option value="12">12-hour (AM / PM)</option><option value="24">24-hour</option></select></label>
    </div>
    <h2 className="font-heading text-xl font-semibold mb-2">Where is everyone working?</h2>
    <p className="text-sm text-quiet mb-4">Set each city's local working hours. The date above follows the first city; every result shows the local date.</p>
    <div className="divide-y divide-line">
      {plan.cities.map((c, i) => <div key={c.name} className="grid grid-cols-2 sm:grid-cols-[1fr_9rem_9rem_3rem] gap-3 py-4 items-end">
        <div className="col-span-2 sm:col-span-1 flex items-center justify-between gap-2 min-w-0"><div className="min-w-0"><h3 className="font-semibold break-words">{c.name}</h3><p className="text-xs text-quiet break-all">{c.timezone}</p></div><button type="button" aria-label={`Remove ${c.name}`} disabled={plan.cities.length <= 2} onClick={() => update({ cities: plan.cities.filter((_, idx) => idx !== i) })} className="sm:hidden p-3 disabled:opacity-30"><X className="w-5 h-5" /></button></div>
        <label className="text-sm text-quiet space-y-1">Starts<input aria-label={`${c.name} work starts`} type="time" step="900" value={Number.isFinite(c.start) ? timeInput(c.start) : ''} onChange={e => changeHours(i, 'start', e.target.value)} className={inputClass} /></label>
        <label className="text-sm text-quiet space-y-1">Ends<input aria-label={`${c.name} work ends`} type="time" step="900" value={Number.isFinite(c.end) ? timeInput(c.end) : ''} onChange={e => changeHours(i, 'end', e.target.value)} className={inputClass} /></label>
        <button type="button" aria-label={`Remove ${c.name}`} disabled={plan.cities.length <= 2} onClick={() => update({ cities: plan.cities.filter((_, idx) => idx !== i) })} className="hidden sm:flex h-12 items-center justify-center disabled:opacity-30 hover:bg-surface rounded-lg"><X className="w-5 h-5" /></button>
      </div>)}
    </div>
    <div className="flex flex-col sm:flex-row sm:flex-wrap gap-4 sm:items-center mt-4 mb-6">
      <label className="w-full sm:flex-1 min-w-0 max-w-sm text-sm text-quiet space-y-1">Add a city ({plan.cities.length}/5)<select aria-label="Add a meeting city" disabled={plan.cities.length >= 5} value="" onChange={e => { if (e.target.value) update({ cities: [...plan.cities, city(e.target.value)] }); }} className={inputClass}><option value="">Choose a city…</option>{CITY_NAMES.filter(name => !plan.cities.some(c => c.name === name)).map(name => <option key={name}>{name}</option>)}</select></label>
    ...
    {computed.error ? <p role="alert" className="text-amber-800 py-4">{computed.error}</p> : <div className="border-t border-line pt-6">
      ...
      {slot && <>
        ...
        </div><p className="text-xs text-quiet mt-3">Creates a one-time calendar draft. Review it before sending invitations. No calendar access is required.</p>
      </>}
      <button type="button" onClick={share} className={`${actionClass} mt-4`}><Share2 className="w-4 h-4" /> Copy plan link</button>
    </div>}
    {notice && <p role="status" className="text-sm text-pine mt-4">{notice}</p>}
